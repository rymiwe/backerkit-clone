...
RSpec.describe FulfillmentWave, type: :model do
  ...
  describe "validations" do
    ...
    it "should define status enum values" do
      expect(FulfillmentWave.statuses.keys).to include(
  ...
  describe "scopes" do
    ...
    let!(:past_due_wave) { create(:fulfillment_wave, project: project, target_ship_date: Date.yesterday, status: 'in_progress') }
    let!(:upcoming_wave) { create(:fulfillment_wave, project: project, target_ship_date: Date.tomorrow, status: 'planned') }
    let!(:completed_wave) { create(:fulfillment_wave, project: project, target_ship_date: Date.yesterday, status: 'completed') }
    
    it "finds upcoming waves" do
      expect(FulfillmentWave.upcoming).to include(upcoming_wave)
      expect(FulfillmentWave.upcoming).not_to include(past_due_wave, completed_wave)
    end
    
    it "finds past due waves" do
      expect(FulfillmentWave.past_due).to include(past_due_wave)
      expect(FulfillmentWave.past_due).not_to include(upcoming_wave, completed_wave)
    end
  end
  ...
  describe "CRUD operations" do
    ...
    it "creates a valid fulfillment wave" do
      ...
      expect(FulfillmentWave.count).to eq(1)
    end
    ...
    it "reads fulfillment wave attributes" do
      ...
      found_wave = FulfillmentWave.find(wave.id)
      ...
      expect(found_wave.target_ship_date.to_date).to eq((Date.today + 30.days).to_date)
    end
    ...
    it "deletes a fulfillment wave" do
      ...
      expect { wave.destroy }.to change(FulfillmentWave, :count).by(-1)
    end
    ...
    it "deletes associated wave items when deleted" do
      ...
      wave_item = create(:wave_item, fulfillment_wave: wave, reward_item: reward_item)
