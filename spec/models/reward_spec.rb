...
RSpec.describe Reward do
  ...
  describe "validations" do
    it "requires a title" do
      reward = described_class.new(description: "Test description", amount: 100.0, project: project)
      expect(reward.valid?).to be false
      expect(reward.errors[:title]).to include("can't be blank")
    end

    it "requires a description" do
      reward = described_class.new(title: "Test Reward", amount: 100.0, project: project)
      expect(reward.valid?).to be false
      expect(reward.errors[:description]).to include("can't be blank")
    end

    it "requires an amount" do
      reward = described_class.new(title: "Test Reward", description: "Test description", project: project)
      expect(reward.valid?).to be false
  ...
  describe "associations" do
    ...
    it "has many reward_items" do
      association = described_class.reflect_on_association(:reward_items)
  ...
  describe "fulfillment status" do
    it "has a status attribute for tracking fulfillment" do
      reward = described_class.create(title: "Test Reward", description: "Test description", amount: 100.0,
                                      project: project)
  ...
  describe "reward items management" do
    let(:reward) do
      described_class.create(title: "Test Reward", description: "Test description", amount: 100.0, project: project)
    end
