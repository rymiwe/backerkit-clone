...
RSpec.describe Project, type: :model do
  describe "validations" do
    it "requires a title" do
      project = Project.new(description: "Test description", goal: 100.0)
      expect(project.valid?).to be false
      expect(project.errors[:title]).to include("can't be blank")
    end

    it "requires a description" do
      project = Project.new(title: "Test Project", goal: 100.0)
      expect(project.valid?).to be false
  ...
  describe "associations" do
    it "belongs to a creator" do
      association = Project.reflect_on_association(:creator)
      expect(association.macro).to eq :belongs_to
    end

    it "has many rewards" do
      association = Project.reflect_on_association(:rewards)
      expect(association.macro).to eq :has_many
    end

    it "has many pledges" do
      association = Project.reflect_on_association(:pledges)
      expect(association.macro).to eq :has_many
    end
  end
  ...
  describe "status methods" do
    ...
    let(:project) { Project.create(title: "Test Project", description: "Test description", goal: 1000.0, creator: creator, status: 'draft', end_date: 1.month.from_now) }
